...
* **Fleeting Routes**: If you have an active route set from an Entrance signal, you can simply tap the Entrance again, and then tap a *different* valid Exit. LeverFrame will seamlessly cancel the old route and build the new one, automatically dropping the conflicting signals and re-aligning the points.
* **Canceling a Route**: If a route is already set, you can cancel it by double-tapping the **Entrance** button OR by double-tapping the **Exit** button. The system will trace the aligned route, drop the entrance signal to Danger, and safely drop any other intermediate signals it had cleared for that route. 
  * *Note on Points*: By default, canceling a route leaves the points in their last used position to minimize mechanical wear. If you prefer points to automatically revert to Normal upon cancellation, you can enable `default_restore_points: true` in your frame configuration.
