...
- **Documentation**: Comprehensive documentation pass to add missing KDocs to all previously undocumented classes, objects, interfaces, and functions across the `config`, `domain`, `openlcb`, and `ui` modules.
...
### Fixed
- **UI/UX (Touchscreens)**: Fixed missing vertical touch-scrolling in the "System Settings" list on touchscreen devices.
- **UI/UX (Touchscreens)**: Implemented horizontal touch-scrolling (swipe-to-scroll) for the Levers row, Block Shelf, and Schematic views, which previously required a mouse/scrollwheel.
...
- **Network**: Fixed an issue where `LccNode` served un-prefixed short-suffix Event IDs to JMRI's memory map. LeverFrame now intelligently expands them using the application `node_id` prefix before serving them to JMRI, and contracts them cleanly back into `config.json` when JMRI writes to them.
...
- **Architecture**: Extracted all OpenLCB/LCC networking logic (GridConnect, SNIP, Datagrams) into a separate, decoupled `:openlcb` KMP library module with clean interface boundaries.
- **Logic**: Implemented a `RuleValidator` utilizing a hybrid Breadth-First Search (BFS) and Dependency Graph trace to robustly validate interlocking configurations and prevent circular logic.
...
- **Cross-Interlocking**: Levers can now interlock against digital block states (occupied/clear) in addition to other levers.
